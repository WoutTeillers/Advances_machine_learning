...
from src.trainer_mlp_6d import Trainer, EarlyStopping
import os
import pickle
import torch
...
def append_velocities(t, t_n, scaler, deltat, lag, dtype=torch.float32, device='cpu'):
    # Convert input tensor to specified dtype and device
    t = t.to(dtype).to(device)
    t_n = t_n.to(dtype).to(device)
    
    # Ensure consistent shape for scaling
    t = t.unsqueeze(0)
    
    # Convert to numpy with consistent dtype
    t_numpy = t.cpu().numpy().astype(np.float32)
    t_unscaled = scaler.inverse_transform(t_numpy)

    # Handle t_n scaling
    t_n_scaled = np.zeros((1, 12), dtype=np.float32)
    t_n_scaled[:, :6] = t_n.cpu().numpy().astype(np.float32)
    ...
    t_new_vel = np.hstack((t_n_positions, velocities)).astype(np.float32)
    t_new_scaled = scaler.transform(t_new_vel)
    
    # Convert back to tensor with specified dtype and device
    return torch.tensor(t_new_scaled, dtype=dtype, device=device)
...
def generate_timeseries(model, steps, generated, Y_test, criterion, scaler, device='cpu', deltat=1e-3, lag=1):
    """
